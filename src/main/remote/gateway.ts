...
export class RemoteGateway extends EventEmitter {
    ...
    if (this.pairingRequests.has(userKey)) {
      ...
      if (inputCode === existing.code) {
        ...
        await this.sendToChannel({
          ...
          content: {
            type: 'text',
            text: '✅ Paired successfully! You can now start using the bot.',
          },
          replyTo: message.id,
        });
      ...
      if (Date.now() > existing.expiresAt) {
        ...
        await this.sendToChannel({
          ...
          content: {
            type: 'text',
            text: `Please enter the pairing code to verify.\n\nYour pairing code is: **${existing.code}**\n\nSend this code to the administrator for confirmation, or reply directly with the code to complete pairing.`,
          },
          replyTo: message.id,
        });
        return;
  ...
  private handleWebhook(req: IncomingMessage, res: ServerResponse, url: string): void {
    // Extract channel type from URL: /webhook/telegram, etc.
